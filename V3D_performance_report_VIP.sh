...
SUBJECT=`echo "V3D VIP Performance Report for WEEK  ${petsa}" | sed -e 's/_to_/ to /g'`
ATTACH=${REPORT}/v3d_vip_perf_report_${petsa}.tar.gz

echo "`date` Processing..."
summary_daily >  ${REPORT}/V3D_VIP_perf_summary_DAILY_report_${petsa}.csv
summary_bh > ${REPORT}/V3D_VIP_perf_summary_BH_report_${petsa}.csv

cat ${HEADER} ${TRAILER} > ${BODY}

cd ${REPORT}
tar cvf v3d_vip_perf_report_${petsa}.tar V3D_VIP_perf_summary_DAILY_report_${petsa}.csv V3D_VIP_perf_summary_BH_report_${petsa}.csv

gzip -f ${REPORT}/v3d_vip_perf_report_${petsa}.tar
...
rm -f ${REPORT}/V3D_VIP_perf_summary_DAILY_report_${petsa}.csv ${REPORT}/V3D_VIP_perf_summary_BH_report_${petsa}.csv
